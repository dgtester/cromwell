...
class CromwellApiHandler(workflowManager: ActorRef) extends Actor {
  ...
  override def receive = {
    ...
    case WorkflowSubmit(source) => workflowManager ! WorkflowManagerActor.SubmitWorkflow(source)
    case WorkflowManagerSubmissionResponse(workflowId) =>
      workflowId match {
        case Success(id) => context.parent ! RequestComplete(StatusCodes.Created, WorkflowSubmitResponse(id.toString, engine.WorkflowSubmitted.toString))
        case Failure(ex) =>
          ex match {
    ...
    case WorkflowOutputs(id) => workflowManager ! WorkflowManagerActor.WorkflowOutputs(id)
    case WorkflowManagerOutputsResponse(outputs, id) =>
      outputs match {
        case Success(o) => context.parent ! RequestComplete(StatusCodes.OK, WorkflowOutputResponse(id.toString, o.mapToValues))
